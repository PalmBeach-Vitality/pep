...
A fully static, multi-page marketing website (HTML + Tailwind CSS via CDN + a little vanilla JS). Deployed via GitHub Pages (see `CNAME`).
...
- To run it in development, serve the repo root over HTTP (opening `file://` also works but relative links behave best over HTTP):
