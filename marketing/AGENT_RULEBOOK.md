...
- **Talking recipe (locked):** Talking clip = **`pep_lipsync_fal`** OmniHuman v1.5 (`fal-ai/bytedance/omnihuman/v1.5`): **`image_url`** `={{ $('save_still_url').item.json.reel_still_url }}` + **`audio_url`** `={{ $('fal_upload_tts_initiate').item.json.file_url }}`. Not `video_url` (sync-3 / VEED / Kling lipsync / LatentSync — stop). Resolution **`1080p`**. Max Wait **`900`**. Do **not** hardcode Audio Url. Do **not** pin `grok_imagine_reel_still`, `tts_pep_voice_over`, or `pep_lipsync_fal` on a unique-scene run. Unique row = `sort_rotation` (`times_used` ASC, then `last_used_at` ASC) + `Limit` = 1. Kling I2V stays disconnected from this talking path. Keeper: `marketing/n8n-pep-omnihuman-keeper.txt`.
