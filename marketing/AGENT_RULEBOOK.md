...
- **Kling walk+talk lock:** Hard fail is the **mouth**. The cartoon mouth on the 10ml label must move from 0.00s through 15.00s (lipsync needs that motion). A grin still makes Kling freeze the mouth until ~12s — still mouth must be a talking AH/OH, not a held smile. Walk the whole clip is still required. **Sal (2026-08-13): thumbs-up is not a blocker.** Do not reroll a still or clip only to remove thumbs-up. Paste `marketing/n8n-pep-prep-video-beat.js` into `prep_grok_video_start`. `cfg_scale` `0.8`. Re-run the still when the mouth is a held grin. Paste `marketing/n8n-pep-grok-still-body-lock.txt` into `grok_imagine_reel_still`.
