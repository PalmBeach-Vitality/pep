...
- **Kling talking lock:** Beat A I2V must look like Pep is talking **the entire 15s**. Mouth on the 10ml label starts at 0.00s and never stops for a smile/pose. No thumbs-up, hat tip, or grin-hold in the Kling prompt (those freeze the mouth). Paste `marketing/n8n-pep-prep-video-beat.js` into `prep_grok_video_start`. `cfg_scale` `0.7`. Do not re-run stills unless the picture is wrong.
